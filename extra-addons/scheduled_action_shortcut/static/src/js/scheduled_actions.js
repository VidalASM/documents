 ...
 const { Component, xml, onMounted, useExternalListener, useRef, useEffect, onWillDestroy } = owl;
/**
        ...
        //  }).then(function(data) {
            ...
            //  } else if (data.length > 0) {
                //  data.forEach(function(res) {
                     var scheduled_action_date = moment.utc().local();
                     var formatted_date = scheduled_action_date.format('DD-MM-YYYY HH:mm:ss');
                     div_body.append("<div class='cron_container'><div><p class='cron_name p_class'>" + "Escáner" + "</p><p class='cron_nextcall p_class'>Fecha : " + formatted_date + "</p></div><button id=" + 1 + " class='cron_btn btn-primary'>Abrir</button></div>")
                //  });
            //  }
        //  })
